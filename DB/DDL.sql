...
(
  ...
  tel_number     VARCHAR(15)  NOT NULL UNIQUE COMMENT '사용자 폰번호',
  address        TEXT         NOT NULL COMMENT '사용자 주소',
  address_detail TEXT         NULL     COMMENT '사용자 상세주소',
  profile_image  TEXT         NULL     COMMENT '사용자 프로필사진',
  PRIMARY KEY (email)
) COMMENT '사용자 테이블';
...
LEFT JOIN (SELECT board_number, MAX(image) AS image FROM image GROUP BY board_number) AS I
